...
Each device defines its own channels and shadow channels:

- Channels are defined with :c:macro:`ZBUS_CHAN_DEFINE` and added to proxy agents with :c:macro:`ZBUS_PROXY_ADD_CHANNEL`
- Shadow channels are defined with :c:macro:`ZBUS_SHADOW_CHANNEL_DEFINE` and automatically mirror channels from other devices
