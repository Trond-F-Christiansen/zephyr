...
struct zbus_proxy_agent_uart_rx_config {
	...
	/** Size of each RX buffer fragment */
	size_t async_rx_buf_size;

	/** Number of RX buffer fragments */
	size_t async_rx_buf_count;
...
struct zbus_proxy_agent_uart_callback_config {
	/** Callback function for received raw data */
	int (*recv_cb)(uint8_t *data, size_t length, void *user_data);

	/** User data for the raw receive callback */
...
#define _ZBUS_GENERATE_BACKEND_CONFIG_ZBUS_PROXY_AGENT_TYPE_UART(_name, _node_id)                  \
	static uint8_t _name##_uart_rx_bufs[DT_PROP(_node_id, rx_buf_count)]                       \
					   [DT_PROP(_node_id, rx_chunk_size)];                     \
	...
	static struct zbus_proxy_agent_uart_config _name##_uart_config = {                         \
		.dev = DEVICE_DT_GET(DT_PHANDLE(_node_id, uart_device)),                           \
		.rx =                                                                              \
			{                                                                          \
				.async_rx_buf = _name##_uart_rx_bufs,                              \
				.async_rx_buf_size = DT_PROP(_node_id, rx_chunk_size),             \
				.async_rx_buf_count = DT_PROP(_node_id, rx_buf_count),             \
				.buf_idx = 0,                                                      \
				.rx_buf_timeout_us = DT_PROP(_node_id, rx_buf_timeout_us),         \
				.fsm =                                                             \
					{                                                          \
