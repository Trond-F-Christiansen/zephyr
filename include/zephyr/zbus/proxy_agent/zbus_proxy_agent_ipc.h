...
/**
 * @brief Instantiate an IPC proxy agent from device tree node.
 *
 * @param node_id Device tree node identifier passed by DT_FOREACH_STATUS_OKAY
 */
#define ZBUS_PROXY_AGENT_INSTANTIATE_IPC(node_id)                                                  \
	ZBUS_PROXY_AGENT_DEFINE(node_id, zbus_##node_id, ZBUS_PROXY_AGENT_TYPE_IPC)
 ...
 * @param _node_id The device tree node ID for the IPC device.
 */
#define _ZBUS_GENERATE_BACKEND_CONFIG_ZBUS_PROXY_AGENT_TYPE_IPC(_name, _node_id)                   \
	static struct ipc_ept_cfg _name##_ipc_ept_cfg = {                                          \
		.name = "ipc_ept_" #_name,                                                         \
	};                                                                                         \
	static struct zbus_proxy_agent_ipc_config _name##_ipc_config = {                           \
		.dev = DEVICE_DT_GET(DT_PHANDLE(_node_id, ipc_device)),                            \
