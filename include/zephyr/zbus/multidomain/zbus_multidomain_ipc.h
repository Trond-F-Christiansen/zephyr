...
/**
 * @brief Function to initialize the IPC backend.
 *
 * This function sets up the IPC endpoint configuration, opens the IPC instance,
 * registers the IPC endpoint, and waits for the endpoint to be bound.
 *
 * @param config void pointer to the IPC configuration structure.
 * @return int 0 on success, negative error code on failure.
 */
int zbus_multidomain_ipc_backend_init(void *config);

/**
 * @brief Function to send a message through the IPC backend.
 *
 * This function sends a message through the IPC endpoint. It checks for valid
 * message size and handles errors during the send operation.
 *
 * @param config void pointer to the IPC configuration structure.
