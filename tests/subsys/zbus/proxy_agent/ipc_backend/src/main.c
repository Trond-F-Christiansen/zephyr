...
FAKE_VALUE_FUNC3(int, fake_recv_cb, uint8_t *, size_t, void *);
...
ZTEST(ipc_backend, test_ipc_backend_send_valid)
{
	...
	memcpy(&sent_crc, (uint8_t *)sent_data + sizeof(test_data), sizeof(sent_crc));
...
ZTEST(ipc_backend, test_ipc_backend_receive_crc_error)
{
	...
	uint32_t wrong_crc = 0xDEADBEEF; /* Intentionally wrong */
