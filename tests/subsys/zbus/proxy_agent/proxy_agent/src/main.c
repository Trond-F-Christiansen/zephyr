...
#define PROXY_NODE DT_NODELABEL(test_proxy_agent)
...
ZBUS_PROXY_ADD_CHANNEL(PROXY_NODE, test_channel_1);
ZBUS_PROXY_ADD_CHANNEL(PROXY_NODE, test_channel_2);
ZBUS_SHADOW_CHANNEL_DEFINE(test_shadow_channel_1, uint32_t, PROXY_NODE, NULL, ZBUS_OBSERVERS_EMPTY,
			   ZBUS_MSG_INIT(0));
ZBUS_SHADOW_CHANNEL_DEFINE(test_shadow_channel_2, uint32_t, PROXY_NODE, NULL, ZBUS_OBSERVERS_EMPTY,
			   ZBUS_MSG_INIT(0));
...
/* Helper macro to get the proxy agent config name */
#define PROXY_AGENT_CONFIG_NAME(_node)        PROXY_AGENT_CONFIG_NAME_CONCAT(_node)
#define PROXY_AGENT_CONFIG_NAME_CONCAT(_node) zbus_##_node##_config

/* Helper macro to get max attempts from device tree */
#define GET_MAX_ATTEMPTS DT_PROP(PROXY_NODE, ack_attempt_limit)
...
{
	extern struct zbus_proxy_agent_config PROXY_AGENT_CONFIG_NAME(PROXY_NODE);
	struct zbus_proxy_agent_config *config = &PROXY_AGENT_CONFIG_NAME(PROXY_NODE);
	uint32_t total = 0;
	...
	for (uint8_t i = 0; i < attempts; i++) {
		...
		if (i >= 32 || config->tracking.ack_timeout_start_ms > (UINT32_MAX >> i)) {
...
{
	/* Verify the proxy agent was instantiated through device tree macro system */
	/* The proxy agent config structure is created by ZBUS_PROXY_AGENT_DEFINE */
	extern struct zbus_proxy_agent_config PROXY_AGENT_CONFIG_NAME(PROXY_NODE);

	zassert_not_null(&PROXY_AGENT_CONFIG_NAME(PROXY_NODE), "Proxy agent config should exist");
	zassert_not_null(PROXY_AGENT_CONFIG_NAME(PROXY_NODE).backend.backend_api,
			 "Backend API should exist");
	zassert_not_null(PROXY_AGENT_CONFIG_NAME(PROXY_NODE).backend.backend_config,
			 "Backend config should exist");
	zassert_equal((int)PROXY_AGENT_CONFIG_NAME(PROXY_NODE).backend.type,
		      (int)ZBUS_PROXY_AGENT_TYPE_MOCK, "Type should be MOCK");
}
...
ZTEST(proxy_agent_test, test_proxy_agent_backend)
{
	...
	zassert_not_null(PROXY_AGENT_CONFIG_NAME(PROXY_NODE).backend.backend_api->backend_init,
			 "Backend init should not be NULL");
	zassert_not_null(PROXY_AGENT_CONFIG_NAME(PROXY_NODE).backend.backend_api->backend_send,
			 "Backend send should not be NULL");
	zassert_not_null(
		PROXY_AGENT_CONFIG_NAME(PROXY_NODE).backend.backend_api->backend_set_recv_cb,
		"Set recv CB should not be NULL");

	ret = PROXY_AGENT_CONFIG_NAME(PROXY_NODE)
		      .backend.backend_api->backend_init(
			      PROXY_AGENT_CONFIG_NAME(PROXY_NODE).backend.backend_config);
...
ZTEST(proxy_agent_test, test_serialization)
{
	...
	zassert_equal(result, -EINVAL, "Serialization should fail with NULL message");
	result = serialize_proxy_agent_msg(&test_msg, NULL, sizeof(buffer));
	zassert_equal(result, -EINVAL, "Serialization should fail with NULL buffer");

	result = serialize_proxy_agent_msg(&test_msg, small_buffer, sizeof(small_buffer));
	zassert_equal(result, -ENOMEM, "Serialization should fail with buffer too small");
	...
	zassert_equal(ret, -EINVAL, "Deserialization should fail with NULL buffer");
	ret = deserialize_proxy_agent_msg(buffer, result, NULL);
	zassert_equal(ret, -EINVAL, "Deserialization should fail with NULL message");

	ret = deserialize_proxy_agent_msg(buffer, 5, &deserialized_msg);
	zassert_equal(ret, -EINVAL, "Deserialization should fail with insufficient buffer size");
...
ZTEST(proxy_agent_test, test_retransmission_timeout)
{
	...
	uint32_t test_data = 0xDEADBEEFU;
	uint32_t expected_attempts = DT_PROP(DT_NODELABEL(test_proxy_agent), ack_attempt_limit);
	...
	k_sleep(K_MSEC(get_total_timeout(GET_MAX_ATTEMPTS + 1) + 10));
...
ZTEST(proxy_agent_test, test_pool_exhaustion_recovery)
{
	...
	set_ack_mode(MOCK_BACKEND_ACK_MODE_AUTO);
	k_sleep(K_MSEC(get_total_timeout(GET_MAX_ATTEMPTS + 1)));
