...
LOG_MODULE_REGISTER(zbus_proxy_agent_ipc, CONFIG_ZBUS_PROXY_AGENT_LOG_LEVEL);
...
static void zbus_proxy_agent_ipc_recv_callback(const void *data, size_t len, void *config)
{
	...
	memcpy(&received_crc, (uint8_t *)data + payload_len, sizeof(received_crc));

	calculated_crc = crc32_ieee((uint8_t *)data, payload_len);
	...
	if (ipc_config->recv_cb) {
		int ret = ipc_config->recv_cb((uint8_t *)data, payload_len,
...
static int zbus_proxy_agent_ipc_backend_set_recv_cb(void *config,
						    int (*recv_cb)(uint8_t *data, size_t length,
								   void *user_data),
						    void *user_data)
{
	struct zbus_proxy_agent_ipc_config *ipc_config =
...
DT_FOREACH_STATUS_OKAY(zephyr_zbus_proxy_agent_ipc, ZBUS_PROXY_AGENT_INSTANTIATE_IPC);
