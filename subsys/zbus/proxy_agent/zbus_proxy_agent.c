...
static int zbus_proxy_agent_stop_retransmission(struct zbus_proxy_agent_config *config,
						uint32_t msg_id);
...
static int handle_recv_ack(struct zbus_proxy_agent_config *config, uint32_t msg_id)
{
	...
	ret = zbus_proxy_agent_stop_retransmission(config, msg_id);
	if (ret < 0) {
		if (ret == -ENOENT) {
...
static int recv_callback(uint8_t *data, size_t length, void *user_data)
{
...
static void zbus_proxy_agent_ack_timeout_handler(struct k_work *work)
{
	...
	if ((data->transmit_attempts < data->config->tracking.ack_attempt_limit) ||
		...
		LOG_WRN("Retrying to send message ID %d (attempt %d)", expected_msg_id,
			data->transmit_attempts);
		if (data->config) {
			ret = zbus_proxy_agent_send(data->config, &data->msg,
						    data->transmit_attempts);
			if (ret < 0) {
		...
		if (data->config) {
			ret = zbus_proxy_agent_stop_retransmission(data->config, expected_msg_id);
			if (ret < 0 && ret != -ENOENT) {
...
static int zbus_proxy_agent_start_retransmission(struct zbus_proxy_agent_config *config,
						 struct zbus_proxy_agent_msg *msg,
						 uint8_t transmit_attempts)
{
	...
	data = zbus_proxy_agent_find_sent_msg_data(config, msg->id);
	if (data) {
		...
		timeout_ms = calculate_backoff_timeout(config, transmit_attempts);
				 ...
				 struct zbus_proxy_agent_msg *msg, uint8_t transmit_attempts)
{
	...
	if (config->tracking.tracking_msg_pool) {
		ret = zbus_proxy_agent_start_retransmission(config, msg, transmit_attempts);
		if (ret < 0) {
	...
	if (ret < 0) {
		...
		if (config->tracking.tracking_msg_pool) {
			cleanup_ret = zbus_proxy_agent_stop_retransmission(config, msg->id);
			if (cleanup_ret < 0) {
