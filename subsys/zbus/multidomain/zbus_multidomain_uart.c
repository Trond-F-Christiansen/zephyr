...
int zbus_multidomain_uart_backend_ack(void *config, uint32_t msg_id)
{
	...
	ret = uart_tx(uart_config->dev, (uint8_t *)&ack_msg, sizeof(ack_msg), SYS_FOREVER_US);
	if (ret < 0) {
					      ...
					      int (*recv_cb)(struct zbus_proxy_agent_msg *msg))
{
	...
	if (uart_config == NULL || recv_cb == NULL) {
					     ...
					     void *user_data)
{
	...
	if (uart_config == NULL || ack_cb == NULL) {
